...
const token = $.isNode() ? process.env.kfxtoken : ""
...
function traceEvent (token, body) {
  return new Promise(async (resolve, reject) => {
    let url = `https://fscrm.kraftheinz.net.cn/crm/public/index.php/api/v1/traceEvent`
    let body = ``
    let urlObject = populateUrlObject(url, token, body)
    await httpRequest('post', urlObject)
    let result = httpResult;
...
function exchangeFigt (token) {
  return new Promise(async (resolve, reject) => {
    let url = `https://fscrm.kraftheinz.net.cn//crm/public/index.php/api/v1/exchangeIntegralNew`
    let body = `value=%E4%BA%AC%E4%B8%9CE%E5%8D%A12%E5%85%83&phone=&type=%E8%A7%86%E9%A2%91%E5%8D%A1`
    let urlObject = populateUrlObject(url, token, body)
    await httpRequest('post', urlObject)
    let result = httpResult;
    console.log(result ? result.msg : "")
...
function getScoreOrder(token,body){
  return new Promise(async (resolve, reject) => {
    ...
    for(let i = 0; i < respData.length; i++){
      cards.push(respData[i].cards.card)
    }
...
function populateUrlObject (url, token, body = '') {
  ...
  let urlObject = {
    ...
    headers: {
      ...
      'User-Agent': "Mozilla/5.0 (iPhone; CPU iPhone OS 15_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.25(0x1800192b) NetType/4G Language/zh_CN",
